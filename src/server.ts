...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext) {
    ...
    // Admin agent avatars — generated by the admin agent and stored in its
    // per-workspace DO (`admin:{wsId}`). DOs aren't publicly reachable, so this
    // route forwards to the right instance, which returns the bytes (or 404)
    // with a long immutable cache. Path: /icons/admin/{wsId}/{key}.{ext}
    const icon = url.pathname.match(/^\/icons\/admin\/(\d+)\/.+$/);
