...
import { getAdminIconUrl } from "@/db/models/workspace-configs";
...
export async function resolveTargets(
  ...
  // shared registry row, so its workspace-specific avatar (generated by the admin
  // agent and kept in workspace_configs) overrides the row's iconUrl here.
  if (ws) {
    const admin = await getAgent(db, "admin");
    if (admin?.enabled) {
      const iconUrl = await getAdminIconUrl(db, ws.id);
      add(iconUrl ? { ...admin, iconUrl } : admin, ws.id);
    }
  }
