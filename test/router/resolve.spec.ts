...
beforeEach(async () => {
  ...
  await env.DB.prepare(
    "INSERT OR IGNORE INTO agents (name, kind, enabled, workspace_id, a2a_endpoint, tenant_id, notify_on) VALUES ('weather','custom',1,0,'https://example.com/weather','main','mention')"
  ).run();
  await env.DB.prepare(
    "INSERT OR IGNORE INTO agents (name, kind, enabled, workspace_id, a2a_endpoint, tenant_id, notify_on) VALUES ('sales','custom',1,0,'https://example.com/sales','main','channel_messages')"
  ).run();
  await env.DB.prepare(
    "INSERT OR IGNORE INTO agents (name, kind, enabled, workspace_id, a2a_endpoint, tenant_id, notify_on) VALUES ('off','custom',0,0,'https://example.com/off','main','channel_messages')"
  ).run();
